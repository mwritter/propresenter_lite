    ...
    <section className="bg-neutral-800">
        <div className="mb-2">
          ...
          <ul className="list-none grid">
            {library ? library.children.map((file) => (
                  <li
                    onClick={() => {
                        if (!isCurrentFile(file)) setCurrentFile(file)
                    }}
                    key={file.name}
                    className={`${isCurrentFile(file) ? 'bg-gray-400' : '' } px-2 py-1 text-xs hover:bg-gray-400 hover:cursor-pointer`}
                  >
                    {file.name}
                  </li>
                ))
...
export default connect(mapStateToProps, mapDispatchToProps)(LibraryContentsView)